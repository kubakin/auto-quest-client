...
const Timer: FC<ITimerComponent> = ({ timerTo = new Date("Sep 16, 2021 16:34:25") }: ITimerComponent) => {
    const end = timerTo.getTime();
    const date = new Date();
    const [dist, setDist] = useState((end - date.getTime()) / 1000);
    const [timer, setTimer] = useState<ITimerFormat>({day: '00', hour: '00', min: '00', sec: '00'});
    useEffect(() => {
